...
# Function to create sequences with a specified length
def create_sequences(hourly_data, labels, sequence_length):
    X, y = [], []
    for i in range(len(hourly_data) - sequence_length - 1):  # Stop one hour early for next-hour prediction
        X.append(hourly_data[i:i + sequence_length])  # Sequence of features for `sequence_length` hours
        y.append(labels[i + sequence_length])         # Label based on the next hour after the sequence
...
def test_model_on_cyclone(test_cyclone_name, test_folder, start_date, end_date):
    ...
    X_test, y_test = create_sequences(hourly_data, labels, sequence_length)
    ...
    with torch.no_grad():
        ...
        all_true_labels.extend(y_test_tensor.cpu().numpy())
        all_predicted_labels.extend(predicted_labels.cpu().numpy())
        
        # Calculate and print accuracy for this test cyclone
        accuracy = accuracy_score(y_test_tensor.cpu(), predicted_labels.cpu())
        print(f"Accuracy on {test_cyclone_name}: {accuracy * 100:.2f}%")
...
if __name__ == "__main__":
    ...
    for cyclone_name, folder in cyclone_data_folders.items():
        ...
        # Create sequences and labels for RNN
        X, y = create_sequences(hourly_data, labels, sequence_length)
        X_tensor = torch.tensor(X, dtype=torch.float32).to(device)  # Move to GPU
        y_tensor = torch.tensor(y, dtype=torch.float32).to(device)  # Move to GPU
        ...
        print(f"Finished training on {cyclone_name}.")
    
    # Save the final model after training on all cyclones
    torch.save(model.state_dict(), 'cyclonePredictor.pth')
    print("Final model saved as 'cyclonePredictor.pth'.")

    # model.load_state_dict(torch.load('cyclonePredictor.pth'))
    # print("Model loaded for testing.")


    # Test on each cyclone in the test set
    ...
    # Calculate overall metrics on the combined test set
    final_accuracy = accuracy_score(all_true_labels, all_predicted_labels)
    final_precision = precision_score(all_true_labels, all_predicted_labels)
    final_recall = recall_score(all_true_labels, all_predicted_labels)
    final_f1 = f1_score(all_true_labels, all_predicted_labels)

    print("\nFinal Metrics Across Both Test Cyclones:")
    print(f"Overall Accuracy: {final_accuracy * 100:.2f}%")
    print(f"Overall Precision: {final_precision:.2f}")
    print(f"Overall Recall: {final_recall:.2f}")
