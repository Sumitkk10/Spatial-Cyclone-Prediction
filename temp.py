...
prediction_window = 24       # Next 24-hour prediction
...
def create_sequences(hourly_data, labels, sequence_length, prediction_window=24):
    X, y = [], []
...
def test_model_on_cyclone(test_cyclone_name, test_folder, start_date, end_date):
    ...
    with torch.no_grad():
        ...
        first_12_accuracy = accuracy_score(y_test_tensor[:, :12].cpu().numpy().flatten(),
                                           predicted_labels[:, :12].cpu().numpy().flatten())
        next_12_accuracy = accuracy_score(y_test_tensor[:, 12:].cpu().numpy().flatten(),
                                          predicted_labels[:, 12:].cpu().numpy().flatten())
        
        print(f"Accuracy on {test_cyclone_name} (First 12 hours): {first_12_accuracy * 100:.2f}%")
        print(f"Accuracy on {test_cyclone_name} (Next 12 hours): {next_12_accuracy * 100:.2f}%")
